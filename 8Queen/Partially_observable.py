...
class PAR_OBS:
        ...
        for state in [belief_state]:
            for col in range(8):
                if (board.Is_Safe(state, row, col)):
                    new_state = frozenset(list(state) + [(row, col)])
    ...
    def Solve(self, board, hint = (0, 0)):
        inital_state = frozenset([hint])
        queue = deque([(inital_state, 1)])
        explored = {inital_state}
        index = 0
        paths = {inital_state: None}
        full_state = []
        solutions = []
